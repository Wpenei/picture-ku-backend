...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public LoginUserVO userLogin(String email, String userPassword, HttpServletRequest request){
        ...
        // 异步添加过期时间key
        //extracted(request, loginUser);
        // 5.获取脱敏后的用户信息并返回
        return getLoginUserVO(loginUser);
    }

    //@Async
    //public  void extracted(HttpServletRequest request, User loginUser) {
    //    HttpSession session = request.getSession();
    //    String sessionId = session.getId();
    //    String sessionKey = "spring:session:sessions:" + sessionId;
    //
    //    try {
    //        // 从 Redis 的 Hash 中获取 expiryTime 字段
    //        Long expiryTime = (Long) stringRedisTemplate.opsForHash().get(sessionKey, "expiryTime");
    //        // 1 分钟 = 60,000 毫秒
    //        long interval = TimeUnit.MINUTES.toMillis(1);
    //        // 向上取整
    //        long roundedExpiry = (expiryTime + interval - 1) / interval * interval;
    //        System.out.println("roundedExpiry过期时间为:" + roundedExpiry);
    //    }catch (NullPointerException e){
    //        log.info("sessionKey 不存在");
    //        throw new BusinessException(ErrorCode.PARAMS_ERROR,"Redis获取过期时间失败");
    //    }
    //    // 获取当前时间戳的毫秒值
    //    long createTime = System.currentTimeMillis();
    //    System.out.println("createTime:" + createTime);
    //    // （创建时间 + 过期时间） 并四舍五入 获取最终过期时间
    //    long sessTimeout = sessionTimeout * 1000L;
    //    System.out.println("sessTimeout:" + sessTimeout);
    //    long finalTime = ((createTime + sessTimeout) / 60000) * 60000;
    //    // 获取登录过期时间
    //    String finalExpirationTime = String.valueOf(finalTime);
    //    System.out.println("finalExpirationTime:" + finalExpirationTime);
    //    // 同时记录当前时间戳
    //    String loginTimestamp = String.format("%s%s", USER_LOGIN_TIMESTAMP, loginUser.getId());
    //    stringRedisTemplate.opsForValue().set(loginTimestamp, finalExpirationTime,sessionTimeout, TimeUnit.SECONDS);
    //}
